...
join.common = {
    ...
    check_empty: function (obj, target) {
        var selector = '';
        if (typeof obj === 'object' && !_.isUndefined(obj.target)) {
            var params = arguments[0].data;
        ...
        if ($(target).length > 0 && $.trim($(target).val()) === '') {
            join.common.toggle_msg(selector, join.msg.required)
        } else {
            join.common.toggle_msg(selector, '', false, 'success')
        }
    },
    //사업자 등록번호 체크
    /* [입력 값 경고 메세지 토글]
     p1 : {string} jquery selector (input)
     p2 : {string} message
     p3 : {bool  } show or hide
     p4 : {string} class name(success or failure or undefined(failure)
     p5 : {int   } hide second
     rt : {void}  */
    toggle_msg: function (selector, msg, isShow, cls, second) {
        if (typeof obj === 'object' && !_.isUndefined(obj.target)) {
            var params = arguments[0].data;
            selector = params.selector;
            msg = params.msg;
        ...
        var $msg = $(selector).siblings('.notice_msg');
        if ($msg.length === 0) {
            $msg = $(selector);
        }
        var $p = $('<p></p>');
        ...
        cls = cls || 'failure';
        $p.html(msg);
        $msg.closest('.notice_msg').attr('class', 'notice_msg ' + cls).html($p).toggle(isShow);

        if (!_.isUndefined(second) && second > 0) {
            clearTimeout(join.common.obj_time)
            join.common.obj_time = setTimeout(function () {
                $('.success').hide();
            }, second * 1000)
        }
    },
