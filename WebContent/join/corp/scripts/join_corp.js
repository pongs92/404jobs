...
join.corp = {
    ...
    change_foreign: function (isForeign) {
        ...
        if (typeof join.account !== 'undefined') {
            join.account.change_foreign();
        }
    },
    submit: function () {
        var c = join.common;
        ...
        // 휴대폰 인증        
        if ($("#DI_Code").val().length == 0 && $("#CertifyCode").val().length == 0 && $("#Mem_HPhone").val().length == 0) {
            alert(join.msg.cert);
