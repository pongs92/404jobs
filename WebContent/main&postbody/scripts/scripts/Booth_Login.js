// 부스 로그인 
var lay_Booth_Open = function (_menu, isKkattalk) {
    // isKkattalk: 까톡은 프레임 안에서 레이어를 띄움
    ...
    var _data = {
        ...
        BoothPagePosition: isKkattalk ? "1" : "0"
    }
...
// 부스 로그인_HR
var lay_Booth_HR_Open = function (_menu, isKkattalk) {
    // isKkattalk: 까톡은 프레임 안에서 레이어를 띄움
    // _menu(1) : /booth/booth_gib_read.asp, _menu(2) : /booth/booth_gi_info_read.asp, menu(3) : /Text_User/Resume/Resume_Step1.asp
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            lay_Booth_HR_Frm_Size(isKkattalk);
        },
        error: function (xhr) {
            //				alert(xhr.message);
        }
...
// 프레임 사이즈 - Ajax 호출시
var lay_Booth_Frm_Size = function (isKkattalk) {
    var jQueryheight = 0;
    ...
    if (jQueryobj.attr("src").toLowerCase().indexOf("booth_frame_login.asp") > -1 || jQueryobj.attr("src").toLowerCase().indexOf("m_login_layer") > -1) {
        ...
        lay_Booth_Size_Chg(jQueryheight, isKkattalk);
    }
}

// 프레임 사이즈 - Ajax 호출시
var lay_Booth_HR_Frm_Size = function (isKkattalk) {
    var jQueryheight = 0;
    var jQueryobj = jQuery("#Booth_Login");

    if (jQueryobj.attr("src").toLowerCase().indexOf("booth_frame_login.asp") > -1 || jQueryobj.attr("src").toLowerCase().indexOf("m_login_layer") > -1) {
        jQueryheight = jQueryobj.height();
        lay_Booth_HR_Size_Chg(jQueryheight, isKkattalk);
    }
}

//	프레임 리사이즈
var lay_Booth_Size_Chg = function (_height, isKkattalk) {
    ...
    jQuerydiv.css({
        "position": "absolute",
        "z-index": "999999",
        "left": isKkattalk ? (jQuerydiv.closest("body").width() - jQuerydiv.width()) / 2 : jQueryleft,
        "top": jQuerytop,
        "bottom": "auto"
    });
}
...
//	프레임 리사이즈
var lay_Booth_HR_Size_Chg = function (_height, isKkattalk) {
    var jQuerydiv = jQuery("body").find("div.layerBooth");
    var jQueryobj = jQuery("#Booth_Login");
    ...
    if (isKkattalk) {
        var parentWidth = jQuerydiv.closest("body").width();
        jQuerydiv.css({
            "position": "absolute",
            "z-index": "999999",
            "left": (parentWidth - jQuerydiv.width()) / 2,
            "bottom": 0,
            "top": "auto"
        });
    } else {
...
var login_Send = function () {
    ...
    if (document.form_login.DB_Name.value == "GI") {
        document.form_login.LoginPage.value = "/Login/Login_GI.asp";
    }
    else {
        document.form_login.LoginPage.value = "/Login/Login_GG.asp";
    }
}